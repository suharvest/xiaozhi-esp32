...
void Application::StartListeningAudio() {
    ...
    if (EvaluateUplinkProbe()) {
        // Returning here aborts the listening startup half-way: voice processing
        // may still be running from the previous segment while wake word
        // detection is already off, leaving the device deaf until some later
        // state change happens to fix it. Reset both synchronously.
        audio_service_.EnableVoiceProcessing(false);
        audio_service_.EnableWakeWordDetection(true);
        return;
    }
